...
void UAnimNotify_AngryEnd::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
	...
	if (Monster)
	{
		...
		if (AIController)
		{
			...
			if (BTComponent)
			{
				BTComponent->GetBlackboardComponent()->SetValueAsBool(TEXT("IsAngryEnd"), true);
			}
		}
	}
}
